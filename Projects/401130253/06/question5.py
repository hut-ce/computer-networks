...
class ChatServer:
    ...
    def send_private(self, sender, recipient_name, message):
        ...
        if recipient:
            if recipient_name in self.blocked_users.get(sender_name, []):
                sender.send(f"You have blocked {recipient_name} and cannot send them private messages.".encode('utf-8'))
            elif sender_name in self.blocked_users.get(recipient_name, []):
                sender.send(f"Your message to {recipient_name} was blocked.".encode('utf-8'))
            else:
                try:
            ...
            try:
                ...
                if message.startswith("/private"):
                    ...
                    if len(parts) == 2:
                        ...
                        self.blocked_users[name].append(blocked_user)
                        client.send(f"You have blocked {blocked_user}.".encode('utf-8'))
                    else:
                        client.send("Invalid block command. Use /block <username>.".encode('utf-8'))

                elif message.startswith("/role"):
                    parts = message.split(" ", 2)
                    if len(parts) == 3 and self.roles[name] == "admin":
                        _, target_user, role = parts
                        if target_user in self.roles:
                            self.roles[target_user] = role
                            client.send(f"{target_user}'s role has been updated to {role}.".encode('utf-8'))
                        else:
                            client.send(f"User {target_user} not found.".encode('utf-8'))
                    else:
                        client.send("Invalid role command or insufficient permissions. Use /role <username> <role>.".encode('utf-8'))
        ...
        while self.running:
            try:
                ...
                if self.gui_done:
                    ...
                    self.chat_area.insert(END, decrypted_message + "\n")
                    self.chat_area.config(state='disabled')
