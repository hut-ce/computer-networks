    ...
    while True:
        try:
            ...
            if message.startswith('/private'):
                ...
                name = usernames[idx]
                send_to_all(f'{name}: {message}'.encode('utf-8'), client)
        except:
            idx = clients.index(client)
            clients.remove(client)
            client.close()
            name = usernames[idx]
            send_to_all(f'{name} left the chat.'.encode('utf-8'), client)
...
def accept_clients():
    while True:
        ...
        send_to_all(f'{name} joined the chat!'.encode('utf-8'), client)
